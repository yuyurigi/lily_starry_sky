...
class Star {
  ...
  boolean okToDraw;
  boolean chaste = true;
  int shapeRandom, colorRandom, mnPoints;
  color[] colorData;
  ...
  void expand() {
    ...
    //キャラの上に図形を描かないようにする
    int check = 0;
    for (int i = 0; i < colorAddArray.length; i++) {
      if (myc == colorAddArray[i]) {
        check += 1;
      }
    }
